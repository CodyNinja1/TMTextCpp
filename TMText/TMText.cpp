...
namespace TrackmaniaText
{
    ...
    FormatSpecifierSearchResult FindAllFormatSpecifiers(std::string Str, ColorEdgecaseBehaviour ColorEdgecaseBehaviourType)
    {
        ...
        bool CharInvolvedInColor = false;
        ...
        for (size_t CharIdx = 0; CharIdx < Str.length(); CharIdx++)
        {
            ...
            {
                if (CharInvolvedInColor && !TrackmaniaText_IsHexadecimal(Char) && ColorEdgecaseBehaviourType == ColorEdgecaseBehaviour_FillZero)
                {
                    while (Temp.length() <= 3)
                    {
                        Temp.push_back('0');
